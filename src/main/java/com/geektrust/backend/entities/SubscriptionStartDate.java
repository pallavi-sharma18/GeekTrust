package com.geektrust.backend.entities;

public class SubscriptionStartDate extends BaseEntity {
    private final String subscription_start_date;

    public String getSubscription_start_date() {
        return subscription_start_date;
    }

    public SubscriptionStartDate(String subscription_start_date) {
        this.subscription_start_date = subscription_start_date;
    }

    @Override
    public int hashCode() {
        final int prime = 31;
        int result = 1;
        result = prime * result + ((id == null) ? 0 : id.hashCode());
        return result;
    }

    @Override
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (obj == null)
            return false;
        if (getClass() != obj.getClass())
            return false;
        SubscriptionStartDate other = (SubscriptionStartDate) obj;
        if (id == null) {
            if (other.id != null)
                return false;
        } else if (!id.equals(other.id))
            return false;
        return true;
    }
   

    @Override
    public String toString() {
        return subscription_start_date ;
    }

    
}
